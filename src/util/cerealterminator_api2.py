...
def cereal_member():
    member = input("Are you a Cereal Terminator member? (y or n)")
    if member=="y":
        idex = True
        while idex:
            cereal_number = input("Please enter your 4 digit Cereal Number: ")
        ...
        new_member = input("Would you like to become a member? (y or n)")
        if new_member=="y":
            idey = True
            while idey:
                credit_card = input("Please enter your credit card number (last 4 digits): ")
...
def collect_payment(paymentType, bill):    
    ...
    if paymentType == "Credit Card":
        ...
        print(
            "The credit card with the number " + cardNumber + " issued to " + cardHolder + " has been charged with $" + str(
                round(bill.getTotal() + tip, 2))
